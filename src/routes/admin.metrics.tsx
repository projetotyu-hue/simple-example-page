...
function MetricsPage() {
  ...
  const byCountry = (metrics ?? []).reduce<Record<string, number>>((acc, m) => {
    const key = m.country ?? "Desconhecido";
    acc[key] = (acc[key] ?? 0) + 1;
    return acc;
  }, {});

  const byDevice = (metrics ?? []).reduce<Record<string, number>>((acc, m) => {
    const key = m.device ?? "Desconhecido";
    acc[key] = (acc[key] ?? 0) + 1;
    return acc;
  }, {});
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <header className="mb-8 flex items-center justify-between">
        ...
        <button
          onClick={() => refetch()}
          className="h-9 px-3 rounded-md border border-border bg-card text-sm font-medium hover:bg-muted transition-colors flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <BreakdownCard title="Por país" data={byCountry} />
        <BreakdownCard title="Por dispositivo" data={byDevice} />
      </div>
            ...
            return (
              <div key={key}>
                <div className="flex justify-between text-xs mb-1">
                  <span>{key}</span>
                  <span className="text-muted-foreground tabular-nums">{value} ({pct.toFixed(0)}%)</span>
                </div>
