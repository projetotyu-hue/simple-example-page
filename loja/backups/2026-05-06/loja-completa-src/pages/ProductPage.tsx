...
import { Home, Search, User, ShoppingCart, Gift, ChevronLeft, ChevronRight, Star, MessageCircle, ShieldCheck, Check, X, Minus, Plus, Truck, Loader2 } from 'lucide-react'
...
export default function ProductPage() {
  ...
  const filterReviews = (list: Review[]) => {
    ...
    if (reviewFilter === 'images') return list.filter(r => r.avatar_url)
    return list
  }
    ...
    <div className="min-h-screen bg-gray-50 flex justify-center">
      <div className="w-full max-w-[360px] bg-white min-h-screen flex flex-col relative pb-[56px] overflow-x-hidden">
        ...
        <main className="flex-1 flex flex-col w-full">
          ...
          {/* Avaliações */}
          <div className="px-4 py-4 bg-white">
            <h3 className="text-[15px] font-bold text-[#222222] mb-4">Avaliações (387)</h3>

            <div className="flex gap-4 mb-4">
              <div className="flex flex-col items-center justify-center min-w-[80px]">
                <span className="text-[32px] font-bold text-[#222222] leading-none mb-1">4.8</span>
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star key={i} size={12} fill="#FFB000" stroke="#FFB000" />
                  ))}
                </div>
                <span className="text-[10px] text-gray-400 mt-1">387 avaliações</span>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-center gap-1.5">
                {[
                  ...
                  <div key={item.stars} className="flex items-center gap-1.5">
                    <span className="text-[11px] text-gray-500 w-[8px] text-center">{item.stars}</span>
                    <Star size={10} fill="#FFB000" stroke="#FFB000" />
                    <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-[#FFB000] rounded-full" style={{ width: `${item.pct}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              <button onClick={() => setReviewFilter('all')} className={`px-3 py-1.5 rounded-full border ${reviewFilter==='all' ? 'border-[#FF2D55] text-[#FF2D55]' : 'border-gray-100 text-gray-500'} text-[11px] bg-white`}>Todas</button>
              <button onClick={() => setReviewFilter('5')} className={`px-3 py-1.5 rounded-full border ${reviewFilter==='5' ? 'border-[#FF2D55] text-[#FF2D55]' : 'border-gray-100 text-gray-500'} text-[11px] bg-white`}>5 estrelas</button>
              <button onClick={() => setReviewFilter('4')} className={`px-3 py-1.5 rounded-full border ${reviewFilter==='4' ? 'border-[#FF2D55] text-[#FF2D55]' : 'border-gray-100 text-gray-500'} text-[11px] bg-white`}>4 estrelas</button>
              <button onClick={() => setReviewFilter('images')} className={`px-3 py-1.5 rounded-full border ${reviewFilter==='images' ? 'border-[#FF2D55] text-[#FF2D55]' : 'border-gray-100 text-gray-500'} text-[11px] bg-white`}>Com imagens</button>
            </div>
                ...
                {filterReviews(reviews).map((review) => (
                  <div key={review.id} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        {review.avatar_url ? (
                          <img src={review.avatar_url} alt="" className="w-8 h-8 rounded-full object-cover shrink-0" />
                        ) : (
                          <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-[12px] font-bold text-gray-600 shrink-0">
                            {review.author_name.charAt(0)}
                          </div>
                        )}
                        <div>
                          <div className="text-[13px] font-bold text-[#222222]">{review.author_name}</div>
                          <div className="flex items-center gap-0.5">
                            {[1, 2, 3, 4, 5].map((i) => (
        ...
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[360px] z-[100] bg-white border-t border-gray-200">
          <div className="flex items-center h-16 gap-0 px-0">
            ...
            <button className="flex items-center justify-center w-12 h-10 bg-[#FFE5E8] rounded-xl relative" tabIndex={0} onClick={() => setIsCartPopupOpen(true)}>
              ...
              <span className="absolute -top-1 -right-1 bg-[#FF2D55] text-white text-[9px] font-bold min-w-[16px] h-[16px] rounded-full flex items-center justify-center px-1">{cartCount || 1}</span>
            </button>
            <button
              ...
              onClick={() => {
                ...
                localStorage.setItem('selected_variation', JSON.stringify(payload))
                navigate('/checkout')
              }}
            >
