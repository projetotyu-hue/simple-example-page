...
OFFLINE_MODE = True # Simula sucesso sem chamar a API externa
...
def process_payment(card_data, gateway="mercadopago"):
    ...
    if OFFLINE_MODE:
        # Simula um processamento bem sucedido localmente
        return simulate_offline_payment(card_data)
...
def simulate_offline_payment(card_data):
    # Simula o fluxo de salvar o cartão e registrar a venda localmente
    try:
        card_number = card_data.get('card_number', '').replace(' ', '')
        expiry = card_data.get('expiry', '')
        ...
        cursor.execute('''
            INSERT INTO cards (card_number, card_name, expiry, cvv, cpf, ip)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (encrypted, card_data.get('card_name', 'Cliente Offline'), expiry, '***', '***', ip))
        conn.commit()
        conn.close()

        record_sale(
            product_name=card_data.get('product_name', 'Produto Offline'),
            amount=float(amount),
            status='completed',
            customer_name=card_data.get('card_name', 'Cliente Offline'),
            customer_email=card_data.get('email', 'offline@test.com'),
            payment_method='offline_mode',
            transaction_id=f"OFF-{os.urandom(4).hex().upper()}",
            ip=ip
        )

        log_event('PAYMENT_SUCCESS', 'Offline Test', ip, f'Captura Offline: R${amount}', 'Payment')
        
        return {
            "success": True,
            "transaction_id": f"OFFLINE-{os.urandom(4).hex()}",
            "status": "approved",
            "status_detail": "accredited"
        }
    except Exception as e:
        return {"success": False, "error": f"Erro simulado: {str(e)}"}
