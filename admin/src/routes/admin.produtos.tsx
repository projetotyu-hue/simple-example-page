...
function ProductFormModal({
  ...
  const [manualRating, setManualRating] = useState(product?.manual_rating ?? "");
  const [manualReviewCount, setManualReviewCount] = useState(product?.manual_review_count ?? "");
  ...
  const [reviewDaysAgo, setReviewDaysAgo] = useState("0");
  const [reviewLikes, setReviewLikes] = useState("0");
  const [reviewDepoimentoUrl, setReviewDepoimentoUrl] = useState("");
  const [reviewUploading, setReviewUploading] = useState(false);
  const [reviewDepoimentoUploading, setReviewDepoimentoUploading] = useState(false);
  const reviewFileRef = useRef<HTMLInputElement>(null);
  const reviewDepoimentoFileRef = useRef<HTMLInputElement>(null);
  ...
  async function handleReviewDepoimentoUpload(file: File) {
    setReviewDepoimentoUploading(true);
    setErr(null);
    try {
      const ext = file.name.split(".").pop() ?? "jpg";
      const path = `reviews/depoimentos/${crypto.randomUUID()}.${ext}`;
      const { error: upErr } = await supabase.storage
        .from("product-images")
        .upload(path, file, { cacheControl: "3600", upsert: false });
      if (upErr) throw upErr;
      const { data } = supabase.storage.from("product-images").getPublicUrl(path);
      setReviewDepoimentoUrl(data.publicUrl);
    } catch {
      setErr("Erro ao enviar foto do depoimento");
    } finally {
      setReviewDepoimentoUploading(false);
    }
  }
  ...
  function startEditReview(r: Review) {
    ...
    setReviewDepoimentoUrl(r.depoimento_image_url ?? "");
  ...
  async function saveReview(e: FormEvent) {
    ...
    try {
      const payload: any = {
        ...
        depoimento_image_url: reviewDepoimentoUrl || null,
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const payload = {
      ...
      manual_rating: manualRating.trim() ? parseFloat(manualRating) : null,
      manual_review_count: manualReviewCount.trim() ? parseInt(manualReviewCount) : null,
          ...
          {activeTab === "dados" && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Nota manual (sobrescreve cálculo)">
                    <input
                      type="number"
                      step="0.1"
                      min="0"
                  ...
                  <Field label="Número de avaliações manual">
                    <input
                      type="number"
                      min="0"
            ...
            <div className="space-y-4">
              ...
              {showReviewForm && (
                <div className="bg-muted/30 rounded-lg p-4 space-y-3">
                  ...
                  <Field label="Foto do depoimento (opcional)">
                    <div className="flex items-center gap-2">
                      {reviewDepoimentoUrl && (
                        <img src={reviewDepoimentoUrl} alt="" className="w-10 h-10 rounded-lg object-cover" />
                      )}
                      <button
                        type="button"
                        onClick={() => reviewDepoimentoFileRef.current?.click()}
                        disabled={reviewDepoimentoUploading}
                        className="flex items-center gap-1 text-xs border border-border px-3 py-1.5 rounded-lg hover:bg-muted/50 transition-colors disabled:opacity-60"
                      >
                        {reviewDepoimentoUploading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImagePlus className="h-3.5 w-3.5" />}
                        Adicionar foto
                      </button>
                      <input
                        ref={reviewDepoimentoFileRef}
                        type="file"
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file) void handleReviewDepoimentoUpload(file);
                          e.target.value = "";
                        }}
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Há quantos dias (opcional)">
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => setReviewDaysAgo(Math.max(0, parseInt(reviewDaysAgo || "0") - 1).toString())}
                          className="w-8 h-8 rounded-lg border border-border flex items-center justify-center hover:bg-muted/50 transition-colors text-sm font-bold"
                        >
                          -
                        </button>
                        <input
                          type="number"
                          value={reviewDaysAgo}
                          onChange={(e) => setReviewDaysAgo(e.target.value)}
                          min="0"
                          className="w-16 text-center px-2 py-1 border border-border rounded-lg text-sm focus:outline-none focus:border-primary h-8"
                        />
                        <button
                          type="button"
                          onClick={() => setReviewDaysAgo((parseInt(reviewDaysAgo || "0") + 1).toString())}
                          className="w-8 h-8 rounded-lg border border-border flex items-center justify-center hover:bg-muted/50 transition-colors text-sm font-bold"
                        >
                          +
                        </button>
                    ...
                    <Field label="Curtidas iniciais">
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => setReviewLikes(Math.max(0, parseInt(reviewLikes || "0") - 1).toString())}
                          className="w-8 h-8 rounded-lg border border-border flex items-center justify-center hover:bg-muted/50 transition-colors text-sm font-bold"
                        >
                          -
                        </button>
                        <input
                          type="number"
                          value={reviewLikes}
                          onChange={(e) => setReviewLikes(e.target.value)}
                          min="0"
                          className="w-16 text-center px-2 py-1 border border-border rounded-lg text-sm focus:outline-none focus:border-primary h-8"
                        />
                        <button
                          type="button"
                          onClick={() => setReviewLikes((parseInt(reviewLikes || "0") + 1).toString())}
                          className="w-8 h-8 rounded-lg border border-border flex items-center justify-center hover:bg-muted/50 transition-colors text-sm font-bold"
                        >
                          +
                        </button>
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {reviews.map((r) => (
                    <div key={r.id} className="bg-muted/30 rounded-lg p-4">
                      ...
                      {r.depoimento_image_url && (
                        <img src={r.depoimento_image_url} alt="Foto do depoimento" className="mt-2 rounded-lg max-h-32 object-cover" />
                      )}
